...
`is_enabled` сделал обязательным намеренно. Умолчание для него опасно в обе стороны: `encoding/json`
игнорирует незнакомые ключи, поэтому опечатка `"enabled"` при умолчании `false` тихо выключит
партнёра, а при `true` — тихо включит и потратит деньги. Обязательное поле — единственный вариант,
при котором сервис не стартует и назовёт проблему.
